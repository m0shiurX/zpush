...
<template>
    ...
    <SetupLayout :current-step="1" :title="hasUsers ? 'Log in to ZPush' : 'Register to ZPush'" :description="hasUsers
        ...
        <template v-if="!hasUsers">
            <div class="flex flex-col gap-6">
                ...
                <!-- Register Form -->
                <Form v-bind="registerStore.form()" :reset-on-success="['password', 'password_confirmation']"
                    :on-success="() => router.visit(device.url())" v-slot="{ errors, processing }"
        ...
        <template v-else>
            ...
            <Form v-bind="loginStore.form()" :reset-on-success="['password']"
                :on-success="() => router.visit(device.url())" v-slot="{ errors, processing }"
